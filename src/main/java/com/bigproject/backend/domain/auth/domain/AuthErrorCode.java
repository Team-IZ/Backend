...
public enum AuthErrorCode implements ApiErrorCode {
	...
	/**
	 * 초대 토큰이 없거나 위변조됐거나, 계정·기관이 링크를 받아 줄 수 없는 상태다.
	 * <b>세부 사유는 알려 주지 않는다</b> — 토큰을 긁어 보는 쪽에 단서가 되기 때문이다.
	 * 만료·이미가입·명단외는 사용자가 다음 행동을 골라야 하므로 아래 코드로 갈라 나간다.
	 */
	INVITATION_INVALID(HttpStatus.BAD_REQUEST, "유효하지 않은 초대 링크입니다."),
	/**
	 * 초대 링크가 만료됐다. 화면은 재발송 안내를 띄운다.
	 * <b>재발송으로 교체된 이전 토큰</b>도 여기로 온다 — 더 새 메일이 이미 도착해 있다는 뜻이라
	 * "문의"보다 "재발송·새 메일 확인" 안내가 맞다.
	 */
	INVITATION_EXPIRED(HttpStatus.GONE, "초대 링크가 만료되었습니다."),
	/**
	 * 이미 수락된 초대다. 만료와 달리 재발송이 아니라 로그인으로 안내해야 한다.
	 * <b>만료보다 먼저 판정한다</b> — 수락된 초대는 시간이 지나면 만료 조건에도 걸리는데,
	 * 그때 사용자에게 맞는 안내는 재발송이 아니라 로그인이기 때문이다.
	 */
