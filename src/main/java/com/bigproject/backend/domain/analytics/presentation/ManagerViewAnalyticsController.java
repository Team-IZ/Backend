...
public class ManagerViewAnalyticsController {
	...
	// =========================================================================
	// MG-01 인박스 행 근거 판정용: 위험 신호 조회 (신규 추가)
	// =========================================================================
	@Operation(
			operationId = "getRiskSignalsForInbox",
			summary = "위험 신호 조회 (인박스 행 근거) | ✅ 사용 가능",
			description = "MG-01 대시보드 및 MG-03, MG-06에서 인박스 행 근거를 조회합니다."
	)
	@GetMapping("/signals")
	public ResponseEntity<RiskSignalResponse> getRiskSignalsForInbox(
		...
		RiskSignalResponse response = service.getRiskSignalsForInbox(authentication.getName(), cohortId, classId, traineeId);
