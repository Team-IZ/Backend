...
public class JdbcOperationalActionQueryRepository implements OperationalActionQueryRepository {
	...
	// =========================================================================
	// 신규 추가: 특정 반(classId) 기준의 조치 필요 항목 조회용 구현
	// =========================================================================

	@Override
	public List<UnassignedClassRow> findUnassignedClassesByClassId(UUID classId) {
		return jdbcTemplate.query(
				"""
                SELECT
                    cl.class_id,
	...
	@Override
	public List<ConceptGapRow> findConceptGapsByClassId(UUID classId) {
		return jdbcTemplate.query(
				"""
                WITH round_scope AS (
                    SELECT
                    ...
                    FROM project_assessment_round r
                    JOIN project p ON p.project_id = r.project_id AND p.deleted_at IS NULL
                    JOIN project_membership pm ON pm.project_id = p.project_id AND pm.class_id = ?
	...
	@Override
	public List<GroupGapRow> findGroupGapsByClassId(UUID classId) {
		return jdbcTemplate.query(
				"""
                WITH round_scope AS (
                    SELECT
	...
	@Override
	public List<InterviewBacklogRow> findInterviewBacklogsByClassId(UUID classId) {
		return jdbcTemplate.query(
				"""
                WITH round_scope AS (
                    SELECT
