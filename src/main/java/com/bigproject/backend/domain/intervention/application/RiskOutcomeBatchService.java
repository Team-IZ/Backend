 ...
 * <h2>트리거 (정책 §5B)</h2>
 *
 * <p>회차 <b>제출 마감</b>이 지났고 그 회차의 <b>마지막 응시 마감 + 1시간 1분</b>도 지나면 그 회차를
 * 한 번에 판정한다. 리포트 발행 이벤트가 아니다 — 발행 시각은 운영자가 미룰 수 있어 판정이 운영
 * 재량에 묶인다.
 *
 * <p>조건이 둘인 이유는 개인 창이 팀 분석 완료 시점부터 개인별로 열리기 때문이다. 회차가 아직
 * 제출을 받고 있어도 먼저 응시한 사람들의 창은 모두 닫힐 수 있고, 그때 판정하면 뒤에 제출한
 * 교육생은 영영 판정되지 않는다(2026-08-18 추가. 근거는
 * {@code JdbcRiskOutcomeBatchRepository.ROUNDS_TO_JUDGE_SQL}에 적었다).
 *
