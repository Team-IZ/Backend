 ...
 */
@Schema(description = "슈퍼어드민 계정 상태 변경 요청")
public record UpdateOperatorStatusForPlatformRequest(

		@Schema(description = "변경할 상태. ACTIVE(재활성) 또는 INACTIVE(정지)만 지정 가능", example = "INACTIVE")
		@NotNull
		OperatorAccountStatus status,

		@Schema(description = "변경 사유(감사 로그용, 선택)", example = "퇴사 처리")
		String reason
) {
